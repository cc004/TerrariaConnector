...
public class Packet
{
    ...
    // just udp packet is used.
    public Packet(){
        this.ip4Header = new IP4Header();
        this.udpHeader = new UDPHeader();
        this.isTCP = false;
        this.isUDP = true;
        this.ip4Header.protocol = IP4Header.TransportProtocol.UDP;
    }
    ...
    public void updateUDPBufferNoChecksum(ByteBuffer buffer, int payloadSize)
    {
        buffer.position(0);
        fillHeader(buffer);

        int udpTotalLength = UDP_HEADER_SIZE + payloadSize;
        buffer.putShort(IP4_HEADER_SIZE + 4, (short) udpTotalLength);
        udpHeader.length = udpTotalLength;

        // Disable UDP checksum validation
        buffer.putShort(IP4_HEADER_SIZE + 6, (short) 0);
        udpHeader.checksum = 0;

        int ip4TotalLength = IP4_HEADER_SIZE + udpTotalLength;
        buffer.putShort(2, (short) ip4TotalLength);
        ip4Header.totalLength = ip4TotalLength;
    }
